...
document.forms[0].addEventListener('submit', ev => {
    ...
    let type = queueType.value, content = queueContent.value || Math.floor(Math.random() * 10000000), priority = queuePriority.checked === true ? 1 : 0
    fetch(`/api/queue/add?type=${type}&content=${content}&priority=${priority}`).then(r => r.json()).then(json => {
