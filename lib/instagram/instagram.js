...
function getStories(username, cookies, userId=null) {
    return new Promise(async (resolve, reject) => {
        ...
        jsonFetch(`https://i.instagram.com/api/v1/feed/user/${userId}/reel_media/`, cookies).then(json => {
            let stories = []
            ...
            for (let item of json.items) {
                ...
                if (story.isVideo) {
                    story.previewUrl = item.image_versions2.candidates[0].url
                    story.mediaUrl = item.video_versions[0].url
                } else {
                    story.mediaUrl = item.image_versions2.candidates[0].url
                }
...
function getSavedPostsAll(cookies, stopAt=null) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        let next = after => {
            getSavedPosts(userId, cookies, after).then(json => {
                for (let node of json.edges) {
                    if (node.node.shortcode === stopAt) {
                        return resolve(posts)
                    } else {
...
function jsonFetch(url, cookies) {
    return new Promise((resolve, reject) => {
        ...
        // console.log('Doing fetch to url:', url)
        fetch(url, { headers })
                .then(r => r.json())
                .then(resolve)
                .catch(reject)
