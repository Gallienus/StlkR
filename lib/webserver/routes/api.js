...
router.get('/user/subscription/update', (req, res) => {
    let username = req.query.username, userId = req.query.userId, loginId = req.query.loginId,
        profile = req.query.profile === 'true' ? 1:0, story = req.query.story === 'true' ? 1:0, posts = req.query.posts === 'true' ? 1:0, interval = req.query.interval
    if (username)  {
    } else if (userId) {
        database.query.get('SELECT * FROM subscription WHERE login_id = ? AND user_id = ?', loginId, userId).then(sub => {
            if (sub) {
                if (!profile && !story && !posts && !interval) {
                    database.query.run('DELETE subscription WHERE login_id = ? AND user_id = ?', loginId, userId).then(() => {
                        res.json({ success:true, userId, loginId, userId })
                    }).catch(console.log)
                } else {
                    console.log('Updating:', profile, story, posts, loginId, userId)
                    database.query.run('UPDATE subscription SET profile=?, story=?, posts=?, interval=? WHERE login_id = ? AND user_id = ?', profile, story, posts, loginId, userId).then(() => {
                        res.json({ success:true, userId, loginId, userId })
                    }).catch(console.log)
                }
            } else {
                database.query.run('INSERT INTO subscription (login_id, user_id, profile, story, posts, interval) VALUES (?,?,?,?,?,?)', loginId, userId, profile, story, posts, parseInt(interval) > 0 ? interval : 84600000).then(() => {
                    res.json({ success:true, userId, loginId, userId })
                }).catch(console.log)
            }
        }).catch(console.log)
