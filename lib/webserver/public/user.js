...
const subElements = [elSubProfile, elSubStory, elSubPosts, elSubInterval, elSubBtn, elUnsubBtn]
elFetchSub.addEventListener('click', () => {
    if (activeLogin) {
        fetch(`/api/user/subscription?userId=${userId}&loginId=${activeLogin.login_id}`).then(r => r.json()).then(json => {
            ...
            if (json.success) {
                subElements.forEach(el => el.disabled = false)
...
elSubBtn.addEventListener('click', () => {
    if (activeLogin) {
        fetch(addParams(`/api/user/subscription/update`, {
            userId,
            loginId: activeLogin.login_id,
            profile: elSubProfile.checked,
            story: elSubStory.checked,
            posts: elSubPosts.checked,
            interval: elSubInterval.value,
