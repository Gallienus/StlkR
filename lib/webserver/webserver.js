...
app.get('/stats', (req, res, next) => {
    database.getStats().then(stats => {
        database.getAverageBirthday().then(averageBirthday => {
            stats.averageBirthday = averageBirthday
...
app.get('/listtest', (req, res, next) => {
    let list = { title:'User list' }, sort = req.query.sort, flow = req.query.flow
    database.query.all(
        `SELECT
            user.*,
            user_real_name.value as real_name,
            count(subscription.login_id) as is_subscribed,
            subscription.profile as profile,
            subscription.story as story,
            subscription.posts as posts,
            subscription.interval as interval
        FROM user
        LEFT JOIN subscription ON subscription.user_id = user.id
        LEFT JOIN user_real_name ON user_real_name.user_id = user.id
        WHERE user.id NOT IN (SELECT user_tag.user_id FROM user_tag WHERE user_tag.value = "hidden")
        GROUP BY user.id
        ORDER BY ${sort || 'id'} ${flow || 'ASC'}`).then(users => {
            list.headers = {
                id: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}` },
                instagram_id: { class:'small' },
                username: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, value: { href:'/user?username=$username$' } },
                real_name: null,
                post_count: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, class:'small' },
                followers_count: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, class:'small' },
                still_exists: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, class:'small', value: { isBool:true } },
                is_private: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, class:'small', value: { isBool:true } },
                is_business: { href:`?sort=id&flow=${ sort === 'id' ? 'desc' : 'asc'}`, class:'small', value: { isBool:true } },
                last_check: { class:'small' },
                is_subscribed: { class:'small', value: { isBool:true } },
                profile: { class:'small', value: { isBool:true } },
                story: { class:'small', value: { isBool:true } },
                posts: { class:'small', value: { isBool:true } },
                interval: { eval:v => {return v/2}, class:'small' },
                '': { noHeader:true, value:{ text:'View posts', href:'/posts?username=$username$', class:'small' }},
                ' ': { noHeader:true, value:{ text:'View contents', href:'/posts?contents=$username$', class:'small' }}
            }
            list.body = users
            res.render('list', { list, navbar })
    }).catch(console.log)
})
