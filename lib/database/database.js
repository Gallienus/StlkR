...
function getAverageBirthday() {
    return new Promise((resolve, reject) => {
        db.prepare(
            `SELECT
                avg(average) as average
            FROM (
                SELECT (birthday_start + birthday_stop) / 2 as average FROM user_age WHERE birthday_stop IS NOT NULL 
                UNION ALL 
                SELECT birthday_start as average FROM user_age WHERE birthday_stop IS NULL
            )`).get((err, row) => {
                if (err) reject(err)
                else resolve(row.average)
        })
    })
}
...
function getStats() {
    return new Promise((resolve, reject) => {
        let stmt = db.prepare(
            ...
            (SELECT count(*) FROM content WHERE is_video = 1) as content_video_count,
            (SELECT count(*) FROM story) as story_count,
            (SELECT count(*) FROM story WHERE is_video = 1) as story_video_count,
            ...
            (SELECT avg(uploaded) FROM post) as average_post_uploaded,
