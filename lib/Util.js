...
function downloadMedia(url) {
    return new Promise((resolve, reject) => {
        if (url && url.length > 0) {
            ...
            request(url).pipe(fs.createWriteStream(path.join(__dirname, '..', config.storageFolder.relativePath, fileName))).on('finish', () => {
                resolve(fileName)
            }).on('error', err => {
                reject(err)
            })
...
module.exports = { moveToTrash, merge, average, object2Arrays, fixNumberInString, downloadMedia, fileNameFromUrl, hashFromFile }