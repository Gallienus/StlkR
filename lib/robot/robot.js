...
async function taskProfile(queueId, loginId, username) {
    try {
        ...
        previous = await (database.query.get('SELECT * FROM user_profile_picture WHERE user_id = ? AND md5 = ?', user.id, md5))
        if (previous)  await (database.update('user_profile_picture', { last_seen:now }, 'id', previous.id))
        else           await (database.set('user_profile_picture', ['user_id', 'url', 'filename', 'md5', 'first_seen', 'last_seen'], [user.id, userData.profile_pic_url_hd, filename, md5, now, now]))
