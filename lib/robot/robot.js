...
async function taskProfile(queueId, loginId, username) {
    try {
        ...
        previous = await (database.query.get('SELECT * FROM user_profile_picture WHERE user_id = ? AND url = ?', user.id, userData.profile_pic_url_hd))
        if (previous)  await (database.update('user_profile_picture', { last_seen:now }, 'id', previous.id))
        else           await (database.set('user_profile_picture', ['user_id', 'url', 'first_seen', 'last_seen'], [user.id, userData.profile_pic_url_hd, now, now]))
        ...
        if (userData.followed_by_viewer && !lfu) {
            // create lfu
            await (database.set('follows', ['user_id', 'login_id', 'login_follows_user'], [user.id, loginId, 1]))
        ...
        if (userData.follows_viewer && !ufl) {
            // create ufl
            await (database.set('follows', ['user_id', 'login_id', 'login_follows_user'], [user.id, loginId, 0]))
...
async function taskSaved(queueId, loginId) {
    try {
        ...
        let codes = await (database.query.all('SELECT code FROM post WHERE id IN (SELECT post_id FROM saved WHERE login_id = ?)', loginId))
        let previousCodes = codes.map(r => r.code)
